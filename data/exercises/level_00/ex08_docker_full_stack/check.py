...
def verify_compose_file_structure(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    for service_name, requirements in Lab08Config.SERVICES.items():
        ...
        if "build_context" in requirements:
            if "build" not in svc:
                errors.append(
                    f"Service '{service_name}' should use 'build:', not 'image:'"
                )
            else:
                build = svc["build"]
                if isinstance(build, str):
                    if build != requirements["build_context"]:
                        errors.append(
                            f"Service '{service_name}' build context should be '{requirements['build_context']}'"
                        )
        ...
        if "image" in requirements:
            if svc.get("image") != requirements["image"]:
                ...
                errors.append(
                    f"Service '{service_name}' should use image '{requirements['image']}', got '{actual}'"
                )
        ...
        if "env_required" in requirements:
            ...
            for req_env in requirements["env_required"]:
                ...
                if not found and req_env not in str(env):
                    errors.append(
                        f"Service '{service_name}' missing environment variable: {req_env}"
                    )
    ...
    if "volumes" not in config:
        ...
        has_db_volume = any(
            "db" in v.lower() and "data" in v.lower() for v in volumes.keys()
        )
...
def verify_frontend_endpoint(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    if expected_content.lower() not in body.lower():
        errors.append(
            f"Frontend response missing expected content: '{expected_content}'"
        )
...
def verify_backend_health(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    if status != expected_status:
        errors.append(
            f"Backend /health returned status {status}, expected {expected_status}"
        )
...
def verify_backend_database_connection(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    if status != expected_status:
        errors.append(
            f"Backend /submissions returned status {status}, expected {expected_status}"
        )
        if status == 500:
    ...
    if expected_content not in body:
        errors.append(
            f"Backend /submissions missing expected data: '{expected_content}'"
        )
...
def verify_volume_exists(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    if not found:
        ...
        errors.append(
            "  → Mount volume to db service: db-data:/var/lib/postgresql/data"
        )
...
def verify_init_sql_mounted(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    code, out, _ = run_cmd(
        ["docker", "exec", container_name, "ls", "/docker-entrypoint-initdb.d/"]
    )
    ...
    if code != 0:
        ...
        errors.append(
            "  → Add bind mount: ./database/init.sql:/docker-entrypoint-initdb.d/init.sql"
        )
...
def verify_network_communication(exercise_path: Path) -> Tuple[bool, List[str]]:
    ...
    code, _, _ = run_cmd(
        ["docker", "exec", backend_container, "ping", "-c", "1", "-W", "2", "db"]
    )
...
def grade(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    lines.append("╔" + "═" * 60 + "╗")
    lines.append(
        "║" + "  Lab 08: The Full Stack - Multi-Service Application  ".center(60) + "║"
    )
    lines.append("╚" + "═" * 60 + "╝")
    lines.append("")
    ...
    lines.append(
        f"   Passing threshold: {Lab08Config.PASSING_PERCENTAGE}% ({passing_score} pts)"
    )
    lines.append("")
...
def grade_smart(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    valid, _, error = compose_config(exercise_path)
    if not valid:
        return grade_checkpoint_compose(code, exercise_path)
    ...
    if running_count < 3:
        return grade_checkpoint_services(code, exercise_path)
