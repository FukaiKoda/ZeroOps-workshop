...
def _run_command(
    args: list[str], timeout: int = 30, cwd: Path = None
) -> Tuple[int, str, str]:
...
def _validate_web_service(
    ...
    if not has_index_mount:
        errors.append(
            "Web service should bind mount ./index.html to /usr/share/nginx/html/index.html"
        )
        errors.append(
            "  → Add: volumes: ['./index.html:/usr/share/nginx/html/index.html']"
        )
...
def _validate_db_service(services: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    if "POSTGRES_PASSWORD" in env or any(
        "POSTGRES_PASSWORD" in str(e) for e in env_list
    ):
...
def _validate_top_level_volumes(
    data: Dict[str, Any],
) -> Tuple[bool, List[str], List[str]]:
    """
...
def _check_stack_running(rendu_path: Path) -> Tuple[bool, List[str], List[str]]:
    ...
    code, out, err = _run_command(
        ["docker", "compose", "ps", "--format", "json"], cwd=rendu_path
    )
