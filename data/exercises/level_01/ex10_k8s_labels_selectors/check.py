...
def grade(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    expected_pods = {
        "web-prod": {"app": "web", "environment": "production", "tier": "frontend"},
        "web-dev": {"app": "web", "environment": "development", "tier": "frontend"},
        "api-prod": {"app": "api", "environment": "production", "tier": "backend"},
    }
        ...
        if name in expected_pods:
            ...
            for label_key, label_value in expected_labels.items():
                if labels.get(label_key) != label_value:
                    errors.append(
                        f"Pod '{name}' should have label '{label_key}: {label_value}'"
                    )
    ...
    try:
        for pod_name, expected_labels in expected_pods.items():
            ...
            if res.returncode != 0:
                return (
                    False,
                    f"YAML is valid, but Pod '{pod_name}' not found in cluster.",
                )
            ...
            for k, v in expected_labels.items():
                if actual_labels.get(k) != v:
                    return (
                        False,
                        f"YAML is valid, but active Pod '{pod_name}' is missing label '{k}: {v}'.",
                    )
            ...
            if phase != "Running":
                return (
                    False,
                    f"YAML is valid, but Pod '{pod_name}' is in '{phase}' state.",
                )
    ...
    return (
        True,
        "✅ Perfect! You've mastered labels and selectors for organizing Kubernetes resources!",
    )
