...
def grade(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    for manifest in documents:
        ...
        if kind == "Secret":
            ...
            try:
                username_decoded = base64.b64decode(data.get("DB_USERNAME", "")).decode(
                    "utf-8"
                )
                if username_decoded != "admin":
                    errors.append(
                        f"DB_USERNAME should decode to 'admin', got '{username_decoded}'"
                    )
            except Exception:
                errors.append("DB_USERNAME is not valid base64 or missing")

            try:
                password_decoded = base64.b64decode(data.get("DB_PASSWORD", "")).decode(
                    "utf-8"
                )
                if password_decoded != "supersecret123":
                    errors.append(
                        f"DB_PASSWORD should decode to 'supersecret123', got '{password_decoded}'"
                    )
            except Exception:
            ...
            if not containers:
                ...
                if not username_from_secret:
                    errors.append(
                        "DB_USERNAME should be loaded from secret 'db-credentials'"
                    )
                if not password_from_secret:
                    errors.append(
                        "DB_PASSWORD should be loaded from secret 'db-credentials'"
                    )
    ...
    try:
        ...
        if not uses_secret:
            return (
                False,
                "YAML is valid, but active Pod does not appear to be using the 'db-credentials' Secret correctly (checked env and envFrom).",
            )
