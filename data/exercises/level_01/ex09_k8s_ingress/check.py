...
def grade(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        for svc_name in ["frontend-service", "api-service"]:
            if (
                subprocess.run(
                    ["kubectl", "get", "service", svc_name], capture_output=True
                ).returncode
                != 0
            ):
                return (
                    False,
                    f"YAML is valid, but Service '{svc_name}' not found in cluster.",
                )

        for dep_name in ["frontend-deploy", "api-deploy"]:
            if (
                subprocess.run(
                    ["kubectl", "get", "deployment", dep_name], capture_output=True
                ).returncode
                != 0
            ):
                return (
                    False,
                    f"YAML is valid, but Deployment '{dep_name}' not found in cluster.",
                )
    ...
    return (
        True,
        "✅ Fantastic! You've mastered Ingress routing to expose multiple services!",
    )
