...
def grade(code: str, exercise_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode != 0:
            return (
                False,
                f"YAML is valid, but Service 'webapp-nodeport' not found. Did you apply it? (Error: {result.stderr.strip()})",
            )
        ...
        if svc_type != "NodePort":
            return (
                False,
                f"YAML is valid, but Service type is '{svc_type}'. Expected 'NodePort'.",
            )
